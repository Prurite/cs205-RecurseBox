...
public:
    ...
    int setParent(int p) { parentId = p; };
...
public:
    Subspace(Map& map, int id, int parentId, size_t len,  bool mirrored, int playerId, string subBoxes) :
    ...
    void loadFromString(stringstream& ss); // Load a subspace from a string
...
public:   
    ...
    void loadFromString(stringstream& ss); // Load a map from a string
